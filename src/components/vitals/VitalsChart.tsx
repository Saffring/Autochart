...
const VitalsChart: React.FC<VitalsChartProps> = ({
  ...
  const chartData = filteredVitals
    .map((vital) => ({
      ...
      systolic: vital.component?.find(
        (c) => c.code.coding[0].display === 'Systolic Blood Pressure'
      )?.valueQuantity?.value,
      diastolic: vital.component?.find(
        (c) => c.code.coding[0].display === 'Diastolic Blood Pressure'
      )?.valueQuantity?.value,
    }))
  ...
  useEffect(() => {
    if (chartRef.current && selectedVital === 'Blood pressure') {
      try {
        ...
        systolicSeries.setData(
          data.map((item) => ({ time: item.time, value: item.high }))
        );

        diastolicSeries.setData(
          data.map((item) => ({ time: item.time, value: item.low }))
        );
        ...
        setError(
          'Failed to create the blood pressure chart. Please try again or select a different vital sign.'
        );
      }
    }
      ...
      return (
        <div className="mt-4 p-4 bg-gray-100 rounded-md">
          ...
          <ul className="list-disc pl-5">
            <li>
              The red line represents Systolic pressure (pressure when the heart
              beats).
            </li>
            <li>
              The blue line represents Diastolic pressure (pressure when the
              heart rests between beats).
            </li>
            <li>
              The colored bars show the range between systolic (top) and
              diastolic (bottom) pressures.
            </li>
            <li>
              Green bars indicate a decrease or no change in systolic pressure
              since the last measurement.
            </li>
            <li>
              Red bars indicate an increase in systolic pressure since the last
              measurement.
            </li>
            <li>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Select a vital sign to view its trend
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
